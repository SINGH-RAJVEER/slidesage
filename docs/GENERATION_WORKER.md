...
The worker has no minimum instance. Cloud Run starts an instance only for an inbound HTTP request, and a committed `river_job` row is not something its autoscaler can observe, so the API sends a wake signal after the submission transaction commits. Signalling before the commit would wake a worker that finds an empty queue and hands its instance straight back.
...
`POST /drain` therefore blocks. It recounts rows in the `generation` queue that are `available`, `running`, `retryable`, or `scheduled`, and returns only once that count has been zero for a settle window, so a job enqueued moments after the last one finishes does not lose its worker to a race. A drain that hits its ceiling still returns `204`: a busy queue is not a failed request, and reporting an error would make Cloud Tasks retry a signal the worker is already acting on.

Because Cloud Tasks dispatches from outside the project network, the worker accepts public ingress. Reachability is gated on IAM alone: only the runtime service account holds `roles/run.invoker`, and there is no `allUsers` binding. The release pipeline asserts the absence of that binding.

Submissions inside one coalescing window share a task name, so a burst of decks wakes the worker once. A name collision means the signal is already queued and is treated as success.
...
The sweep also re-wakes the worker when it finds outstanding queue rows. River schedules a retry for a future time, and a wake signal can be lost while the service is paused for a migration. In both cases the row is waiting and nothing is polling for it, and the sweep is now the only thing running on a timer that can notice. Cloud Scheduler is paused while `maintenance_mode` is set, so a sweep cannot start a worker against a half-migrated schema.
