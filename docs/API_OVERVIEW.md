...
Generated presentation documents do not carry a schema-version field, and the API does not load or translate earlier stored document shapes. Generation binds content to the writable slots a template manifest declares. AI output carries content only: no styling, coordinates, layouts, regions, or CSS. See [PPTX_CANONICAL_FLOW.md](PPTX_CANONICAL_FLOW.md).
