...
async function testEmbeddingGeneration(): Promise<boolean> {
  ...
  try {
    for (const text of testTexts) {
      info(`Generating embedding for: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`);
      ...
      const hasNaN = result.embedding.some((v: number) => typeof v !== 'number' || Number.isNaN(v));
      if (hasNaN) {
...
async function testSearchEmbeddingStorage(): Promise<boolean> {
  ...
  try {
    ...
    error(`Search embedding storage failed: ${err instanceof Error ? err.message : String(err)}`);
    if (err instanceof Error && err.stack) {
      console.error(err.stack);
    }
    return false;
  ...
  try {
    for (const { query, expectedSimilar } of testQueries) {
      ...
      contexts.forEach((ctx, idx) => {
        ...
        console.log(`     ${ctx.context.substring(0, 80)}${ctx.context.length > 80 ? '...' : ''}`);
      });
...
async function testEmbeddingSimilarity(): Promise<boolean> {
  ...
  try {
    ...
    error(`Similarity verification failed: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}
...
async function runTests() {
  ...
  for (const test of tests) {
    try {
      ...
      if (!passed && (test.name === 'LiteLLM Proxy' || test.name === 'Embedding Generation')) {
