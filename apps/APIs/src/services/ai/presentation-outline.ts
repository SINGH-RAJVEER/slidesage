...
export async function generatePresentationOutline(
    ...
    const ttlMs =
        (Number.isFinite(configuredTtl) && configuredTtl > 0 ? configuredTtl : 7 * 24 * 60 * 60) *
        1000;
...
async function requestPresentationOutline(
    ...
    if (!apiKey) {
        throw new OpenRouterStreamError("OpenRouter is not configured. Set OPEN_ROUTER_API_KEY.");
    }
