...
async function initDevUser(secret: string): Promise<boolean> {
  ...
  if (!existingUser) {
    ...
    // Note: Better Auth typically creates accounts through its API, but for dev setup
    // we create it directly. The accountId should match the email for credential auth.
    const hashedPassword = await bcrypt.hash('DevPassword123!', 10);
    await db.insert(accounts).values({
      id: crypto.randomUUID(),
      userId: userId,
      accountId: devEmail, // For credential auth, accountId is typically the email
