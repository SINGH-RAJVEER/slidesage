	...
	const rows = await db
		...
		.where(
			and(
				...
				or(eq(sourceChunks.presentationId, presentationId), isNull(sourceChunks.presentationId)),
				sql`${similarity} > ${threshold}`
			)
		)
		.orderBy(desc(similarity))
...
export async function retrieveTemplateContexts(
	...
	const rows = await db
		...
		.from(slideTemplates)
		.where(
			and(eq(slideTemplates.embeddingModel, embeddingModel), sql`${similarity} > ${threshold}`)
		)
		.orderBy(desc(similarity))
		.limit(limit);
