...
export class AIConnectionService {
	...
	async getConfiguration(userId: string, signal?: AbortSignal): Promise<AIConfigurationResponse> {
		...
		const discoveries = await Promise.all(
			validConnections.map(async (connection) => {
				...
				try {
					...
					if (error instanceof ProviderValidationError && (error.rejected || error.incompatible)) {
						...
						modelCatalogErrors[provider] = "The provider model list is temporarily unavailable.";
					}
		...
		const availableSelections = new Set(models.map((model) => `${model.provider}\0${model.model}`));
		...
		return {
			generation: {
				...
				model:
					validProviders.size > 0 ? validSelection?.model || null : configuredOpenRouterModel(),
	...
	async select(userId: string, selection: AIModelSelection, signal?: AbortSignal): Promise<void> {
		...
		const { models } = await this.discoverModels(userId, selection.provider, connection, signal);
	...
	async resolveSelection(
		...
		for (const provider of AI_PROVIDERS) {
			...
			const { apiKey, models } = await this.discoverModels(userId, provider, connection, signal);
	...
	private defaultSelection(models: AIModelDescriptor[]): AIModelSelection | null {
		for (const provider of AI_PROVIDERS) {
			...
			const model = providerModels.find((candidate) => candidate.recommended) || providerModels[0];
