...
function normalizeEffects(value: unknown): NonNullable<Slide["effects"]> {
	...
	return value
		...
		.map((effect, index) => ({
			id: typeof effect["id"] === "string" ? effect["id"].slice(0, 120) : `effect-${index + 1}`,
...
function normalizeSlide(value: unknown, index: number, used: Set<string>): Slide | null {
	...
	if (raw["type"] === "scene") {
		...
		const variants = Array.isArray(raw["variants"])
			? raw["variants"]
					...
					.map((variant) => {
						...
						const variantRoot = normalizeSceneNode(variant["root"], `${id}-${profile}-root`, 0);
		...
		return {
			...
			root,
			variants,
			strategy: typeof raw["strategy"] === "string" ? raw["strategy"].slice(0, 100) : undefined,
	...
	if ("blocks" in common) {
		const blocks = common.blocks.map((block, blockIndex) => {
			...
			return {
				...
				sourceIds: Array.isArray(rawBlock?.["sourceIds"]) ? rawBlock["sourceIds"] : block.sourceIds,
			} as SlideBlock;
		});
...
function normalizeSceneNode(value: unknown, fallbackId: string, depth: number): SceneNode | null {
	...
	const base = {
		id: typeof raw["id"] === "string" ? raw["id"].trim().slice(0, 120) || fallbackId : fallbackId,
		...
		zIndex: Number.isFinite(Number(raw["zIndex"])) ? Math.round(Number(raw["zIndex"])) : undefined,
		rotation: Number.isFinite(Number(raw["rotation"])) ? Number(raw["rotation"]) : undefined,
		hidden: raw["hidden"] === true,
		optional: raw["optional"] === true,
		sourceIds: Array.isArray(raw["sourceIds"])
			? raw["sourceIds"].filter((item): item is string => typeof item === "string").slice(0, 12)
			: [],
		ariaLabel: typeof raw["ariaLabel"] === "string" ? raw["ariaLabel"].slice(0, 500) : undefined,
	...
	if (type === "group") {
		...
		const children = Array.isArray(raw["children"])
			? raw["children"]
					.slice(0, 120)
					.map((child, index) => normalizeSceneNode(child, `${base.id}-${index + 1}`, depth + 1))
					.filter((child): child is SceneNode => child !== null)
			: [];
		return {
			...base,
			type,
			layout: layout === "absolute" || layout === "grid" || layout === "overlay" ? layout : "stack",
			...
			columns: Array.isArray(raw["columns"])
				? raw["columns"].filter((item): item is number => typeof item === "number").slice(0, 12)
				: undefined,
			rows: Array.isArray(raw["rows"])
				? raw["rows"].filter((item): item is number => typeof item === "number").slice(0, 12)
				: undefined,
			clip: raw["clip"] === true,
			children,
		};
	...
	if (type === "text") {
		...
		return {
			...
			maxLines: Number.isFinite(Number(raw["maxLines"])) ? Number(raw["maxLines"]) : undefined,
	...
	switch (input["type"]) {
		case "update-presentation": {
			...
			return {
				...
				title: typeof input["title"] === "string" ? input["title"].trim().slice(0, 240) : undefined,
				theme: theme as ThemeId | undefined,
				dimensions:
					input["dimensions"] === undefined ? undefined : normalizeDimensions(input["dimensions"]),
			};
		}
		case "update-slide": {
...
export function applyPresentationMutations(
	...
	for (const mutation of mutations) {
		...
		if (mutation.type === "update-slide") {
			...
			if (mutation.slide.id !== mutation.slideId) throw new Error("Slide IDs cannot be changed");
			const used = new Set(
				next.slides.filter((slide) => slide.id !== mutation.slideId).map((slide) => slide.id)
			);
