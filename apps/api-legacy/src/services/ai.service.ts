	...
	async *generatePresentationStream(
		...
		try {
			...
			const generationMemoryContext = userId
				? await this.ragService.buildGenerationMemoryContextString(userId, userPrompt, signal)
				: "";
	...
	async *iteratePresentationStream(
		...
		try {
			...
			const expectedSlideCount =
				slideCount && slideCount > 0 ? slideCount : currentPresentation?.slides.length || undefined;
