			...
			properties: {
				...
				nodes: {
					...
					items: {
						...
						required: ["id", "label", "description", "value", "role", "tone", "parentId"],
						additionalProperties: false,
					},
				},
				edges: {
		...
		try {
			...
			for await (const chunk of readOpenRouterStream(response, {
				...
				for (const { index, slide } of processor.extractSlides()) {
					if (options.expectedSlideCount !== undefined && index >= options.expectedSlideCount) {
						continue;
					}
			...
			const semanticSlides = normalizePresentationSlides(parsedContent, options.expectedSlideCount);
			...
			const completeStreamedSlides =
				expectedSlideCount === undefined
					? []
					: Array.from({ length: expectedSlideCount }, (_, index) => streamedSlides.get(index));
