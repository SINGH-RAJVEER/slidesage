...
function compileContentSlide(
	...
	const density = DENSITY[slide.density];
	const eyebrow = slide.eyebrow ? [textNode(`${slide.id}-eyebrow`, 0, "label", slide.eyebrow)] : [];
	...
	if (layout === "cover") {
		...
		if (variant === "split-cover" && coverVisual) {
			root = {
				...grid(
					...
					[
						{
							...stack(`${slide.id}-cover-copy`, 0, [...header, ...supporting], density.gap),
							grid: { column: 0, row: 0 },
						},
			...
			const copy = stack(`${slide.id}-cover-copy`, 2, [...header, ...supporting], density.gap, {
				bounds: {
					x: centered ? 230 : 76,
					y: centered ? 120 : 126,
		...
		const support = nonMedia.length
			...
			: textNode(`${slide.id}-section-number`, 1, "display", String(index + 1).padStart(2, "0"));
		...
		const quoteHero =
			quote || supporting[0] || textNode(`${slide.id}-quote-empty`, 0, "body", slide.subtitle);
		...
		const supporting = blockNodes
			...
			.map(({ node }) => node);
		const spotlightHero =
			hero || supporting[0] || textNode(`${slide.id}-spotlight-empty`, 0, "body", slide.subtitle);
