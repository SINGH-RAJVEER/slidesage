...
function currentVersion(): number {
	...
	if (!Number.isInteger(version) || version < 1) throw new Error("Invalid BYOK encryption version");
	return version;
}
...
async function encryptionKey(version: number): Promise<CryptoKey> {
	...
	if (raw.byteLength !== 32) throw new Error("BYOK encryption keys must contain exactly 32 bytes");
