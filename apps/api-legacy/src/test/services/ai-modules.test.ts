...
describe("AI presentation content", () => {
	it("normalizes structured blocks, layout regions, and duplicate slide IDs", () => {
		...
		expect(secondSlide?.type === "content" ? secondSlide.blocks[0]?.region : "").toBe("primary");
	});
				...
				blocks: [
					{
						...
						nodes: Array.from({ length: 20 }, (_, index) => ({
							id: `step-${index}`,
							label: `Step ${index}`,
							description: index === 0 ? "https://example.com/generated" : "Milestone",
...
describe("AI presentation messages", () => {
	it("builds generation and iteration messages with shared research context", () => {
		...
		expect(iterationMessages.map((message) => message.role)).toEqual(["system", "system", "user"]);
