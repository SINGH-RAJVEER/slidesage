...
describe("presentation routes", () => {
		...
		try {
			...
			expect(
				presentationUpdates.some(
					({ updates }) =>
						(updates["slidesData"] as { status?: string } | undefined)?.status === "ready"
				)
			).toBe(false);
		} finally {
	...
	it("does not fail or refund a finalized deck when the saved SSE write fails", async () => {
		...
		try {
			...
			expect(
				presentationUpdates.some(
					({ updates }) =>
						(updates["slidesData"] as { status?: string } | undefined)?.status === "failed"
				)
			).toBe(false);
		} finally {
			console.error = originalError;
	...
	it("upserts streamed slides by index before persistence", async () => {
		...
		expect(response.status).toBe(200);
		await text(response);
		const update = presentationUpdates.find(({ updates }) => "slidesData" in updates)?.updates as
	...
	it("paginates lists and reports research for ready and failed decks", async () => {
		...
		expect(body.presentations.map((item: { has_research: boolean }) => item.has_research)).toEqual([
			true,
			true,
		]);
	});
	...
	it("returns, forbids, and deletes presentations by id", async () => {
		...
		presentationService.getPresentation.mockRejectedValueOnce(new Error("Presentation not found"));
