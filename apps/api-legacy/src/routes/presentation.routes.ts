...
function parseResearchOptions(input: unknown): ResearchOptions {
	...
	const includeDomains = parseStringList(value.includeDomains, "research.includeDomains", 10, 253);
	const excludeDomains = parseStringList(value.excludeDomains, "research.excludeDomains", 10, 253);
...
function parseResearchPayload(input: unknown): ResearchPayload {
	...
	for (const [index, source] of sourcesRaw.entries()) {
		...
		sources.push({
			url,
			title: parseOptionalSourceText(src.title, `research_payload.sources[${index}].title`, 500),
			...
			author: parseOptionalSourceText(src.author, `research_payload.sources[${index}].author`, 200),
...
async function markPresentationFailed({
	...
	const retrySlideCount =
		Number.isFinite(parsedSlideCount) && parsedSlideCount > 0 ? Math.floor(parsedSlideCount) : 5;
...
presentations.post(
	...
	async (c) => {
		try {
			...
			const tonality = parseKnownValue(body["tonality"], "tonality", TONALITIES, "professional");
			...
			const researchPayload =
				researchPayloadInput === undefined ? undefined : parseResearchPayload(researchPayloadInput);
			...
			const retryPresentationId =
				typeof retryPresentationIdInput === "string" ? retryPresentationIdInput.trim() : undefined;

			const requestedAI = parseAISelection(body["ai"]);
			let ai: (AIModelSelection & { apiKey: string }) | undefined;
			try {
				ai = await aiConnectionService.resolveSelection(userId, requestedAI, c.req.raw.signal);
			} catch (error) {
			...
			if (retryPresentationId) {
				...
				if (failedData.status !== "failed") {
					return c.json({ error: { message: "Only failed presentations can be retried" } }, 409);
				}
			...
			return trackedStream(c, async (stream) => {
				...
				stream.onAbort(() => cancellation.abort(new DOMException("SSE closed", "AbortError")));
				...
				try {
					...
					for await (const event of presentationService.generatePresentationStream({
						...
						if (eventType === "slide") {
							...
							if (slide) {
								...
								if (Number.isInteger(index) && index >= 0) {
									...
									const existingIndex = allSlides.findIndex((existing) => existing.id === slide.id);
						...
						if (eventType === "error") {
							...
							failureMessage =
								typeof eventData.error === "string" ? eventData.error : failureMessage;
					...
					if (generationFailed || !generationCompleted) {
						if (!generationFailed) {
							failureMessage = "Presentation generation ended before completion. Please try again.";
						}
						await saveFailure(failureMessage);
						if (!generationFailed) {
							await stream.write(
					...
					if (allSlides.length > 0) {
						...
						const chargedTokens = ai
							? 0
							: presentationService.calculateActualTokenCost(tokensUsed, estimatedTokens);
						const finalUpdates = {
							title: finalTitle,
							prompt: topic,
...
presentations.post(
	...
	async (c) => {
		try {
			...
			const topic = parseRequiredText(aliasedField(body, "topic", "prompt", "query"), "topic");
			...
			const generationMode = (await aiConnectionService.getConfiguration(userId)).generation.mode;
			const sources = await searchService.webSearch(topic, research, c.req.raw.signal);

			// Store search embedding for RAG context
			try {
				await searchService.storeSourceChunks(userId, topic, sources, undefined, c.req.raw.signal);
			} catch (error) {
			...
			const quotedSlideCount =
				iterationSlideCount !== undefined
					...
					: Array.isArray(storedSlides) && storedSlides.length > 0
						...
						: 5;
			const estimatedTokens = !ai
				? presentationService.calculateEstimatedTokens(quotedSlideCount, detailLevel, tonality)
				: 0;
			const operationId = crypto.randomUUID();
			let pointReservationActive = false;
			let startingBalance: number;
			...
			return {
				...
				has_research: Boolean(
					(slidesData?.status !== "failed" &&
						...
						(slidesData?.status === "failed" && Array.isArray(retrySources) && retrySources.length)
				),
	...
	async (c) => {
		try {
			...
			const request = parsePresentationMutationRequest(
				(await readJsonBody(c, MAX_MUTATION_BODY_BYTES)) as unknown as PresentationMutationRequest
			);
