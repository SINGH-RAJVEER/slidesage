...
async function refundExpiredReservations(
	...
	for (const operation of expired) {
		...
		const [presentation] = await tx
			.select()
			.from(presentations)
			.where(and(eq(presentations.id, operation.presentationId), eq(presentations.userId, userId)))
			.limit(1);
	...
	async reserveExistingPresentation(
		...
		return await db.transaction(async (tx) => {
			...
			const [presentation] = await tx
				...
				.from(presentations)
				.where(
					and(eq(presentations.id, input.presentationId), eq(presentations.userId, input.userId))
				)
				.limit(1);
