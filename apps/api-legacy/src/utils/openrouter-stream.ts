		...
		if (provider === "google") {
			...
			body = {
				...
				generationConfig: {
					...
					responseJsonSchema: (options.responseFormat["json_schema"] as { schema?: unknown })
						?.schema,
				},
			};
			...
			body = {
				...
				output_config: {
					format: {
						type: "json_schema",
						schema: (options.responseFormat["json_schema"] as { schema?: unknown })?.schema,
					},
				},
			};
		} else {
			...
			body = {
				...
				...(!provider ? { provider: { allow_fallbacks: true, require_parameters: true } } : {}),
			};
		}
	...
	try {
		return await Promise.race([
			...
			new Promise<never>((_resolve, reject) => {
				timeout = setTimeout(
					() => reject(new OpenRouterStreamError(`OpenRouter stream was idle for ${timeoutMs}ms`)),
					timeoutMs
				);
			}),
		]);
...
function parseEventBlock(block: string, anthropicUsage: AnthropicUsageState): StreamChunk | null {
	...
	try {
		...
		if (parsed.type) {
			...
			const outputTokens = parsed.usage?.output_tokens ?? parsed.message?.usage?.output_tokens;
