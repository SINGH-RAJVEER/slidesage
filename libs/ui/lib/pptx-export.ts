	...
	if (node.type === "text") {
		...
		output.addText(node.text || "", {
			...
			fontFace:
				...
				(node.role === "display" || node.role === "title" ? theme.headingFont : theme.bodyFont),
...
const addStructuredList = (
	...
	const estimatedHeight = block.items.reduce(
		(height, item) => height + Math.max(0.34, estimateTextHeight(item, context.region.w, fontSize)),
		0,
	);
...
const renderStructuredSlide = async (
	...
	switch (slideData.layout) {
		case "split":
		case "comparison": {
			const primaryWidth = slideData.layout === "split" ? (width - gap) * 0.59 : (width - gap) / 2;
		...
		case "body": {
			...
			const supportHeight = (height - supportGap * (supporting.length - 1)) / supporting.length;
