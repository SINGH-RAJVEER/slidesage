		...
		<div className="space-y-10">
			<section className="grid gap-5 sm:grid-cols-[1fr_auto] sm:items-center">
				...
				<div className="w-fit rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-right">
					<p className="text-xs text-white/40">{isByok ? "Provider billing" : "Points billing"}</p>
			...
			<section className="border-t border-white/10 pt-10">
				<div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
					...
					{config.models.length > 0 ? (
						...
						>
							...
							<SelectContent className="border-white/10 bg-gray-900 text-white">
								{PROVIDERS.map((provider) => {
									const models = config.models.filter((model) => model.provider === provider.id);
						...
						<p className="text-sm text-white/45">Connect a provider to choose a default model.</p>
					)}
				</div>
			</section>
							...
							<div key={provider.id} className="py-5 first:pt-4 last:pb-4">
								...
								<div className="mt-3 flex flex-col gap-2 sm:flex-row">
									<input
										...
										placeholder={connection ? "Enter replacement key" : "Enter API key"}
