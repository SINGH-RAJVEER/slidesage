...
function QuoteWidget({ node, foreground, accent }: SceneWidgetProps) {
	...
	return (
		...
		>
			...
			{values["attribution"] ? (
				<figcaption className="mt-5 text-sm opacity-60">{String(values["attribution"])}</figcaption>
			) : null}
		</figure>
	);
}
			...
			{items.map((item, index) => {
				...
				return (
					...
					>
						...
						<strong>{title}</strong>
						{description ? <span className="mt-1 text-xs opacity-60">{description}</span> : null}
					</div>
				);
			})}
		</div>
				...
				<input
					...
					onInput={(event) =>
						updateChart({
							data: {
								...chartConfig.data,
								labels: event.currentTarget.value.split("|").map((label) => label.trim()),
							},
						})
					}
				/>
							...
							updateChart({
								data: {
									...
									datasets: chartConfig.data.datasets.map((entry, itemIndex) =>
										itemIndex === index ? { ...entry, label: event.currentTarget.value } : entry,
									),
								},
							})
						}
		...
		return (
			<div className="ss-scene-widget-editor grid gap-2" style={{ color: foreground }}>
				<input
					...
					onInput={(event) =>
						update({
							headers: event.currentTarget.value.split("|").map((item) => item.trim()),
						})
					}
				/>
				<textarea
							...
							onInput={(event) =>
								update({
									items: items.map((entry, itemIndex) =>
										itemIndex === index ? { ...entry, value: event.currentTarget.value } : entry,
									),
								})
							}
						/>
				...
				return (
					<div className="grid grid-cols-2 gap-2" key={`${node.id}-item-${index}`}>
						<input
							...
							onInput={(event) => {
								const next = [...items];
								if (typeof item === "string") next[index] = event.currentTarget.value;
