import type { BlockEmphasis, BlockTreatment, SlideLayout, SlideRegion } from "@slidesage/types";
	...
	return (
		<section className="grid gap-4" aria-label={`${block.type} block editor`}>
			<div className="flex items-center justify-between border-b border-white/10 pb-3">
				<div>
					<p className="text-xs uppercase tracking-[0.18em] text-white/40">Block {index + 1}</p>
				...
				<div className="grid gap-2">
					...
					<Button
						...
						onClick={() =>
							onChange({
								...block,
								items: [...block.items, { value: "0", label: "Metric" }].slice(0, 6),
							})
						}
					>
						Add statistic
						...
						<input
							...
							onChange={(event) => {
								...
								onChange({
									...block,
									headers,
									rows: block.rows.map((row) => headers.map((_, i) => row[i] || "")),
								});
							}}
							...
							onChange={(event) =>
								onChange({
									...
									rows: event.target.value
										.split("\n")
										.slice(0, 8)
										.map((row) => block.headers.map((_, i) => row.split("|")[i]?.trim() || "")),
								})
							}
